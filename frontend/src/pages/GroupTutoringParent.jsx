...
const GroupTutoringParent = () => {
  ...
  const [enrollmentQuizzes, setEnrollmentQuizzes] = useState([]);
  ...
  const handleEnrollmentClick = async (enrollment) => {
    // Only show details for enrolled students
    ...
    try {
      const [filesRes, quizzesRes] = await Promise.all([
        api.get(`/api/group-tutoring/student-files/${enrollment.id}/`),
        api.get(`/api/group-tutoring/student-quizzes/${enrollment.id}/`)
      ]);
      setEnrollmentFiles(filesRes.data);
      setEnrollmentQuizzes(quizzesRes.data);
    } catch (err) {
  ...
  return (
    <div style={{ minHeight: '100vh' }}>
      ...
      {showEnrollmentDetail && selectedEnrollment && (
        <div style={{
          position: 'fixed',
          top: 0,
          left: 0,
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.5rem' }}>
              <div>
                <h2 style={{ margin: '0 0 0.5rem 0', color: '#192A88' }}>
                  {selectedEnrollment.class_title}
                </h2>
                <p style={{ margin: 0, color: '#666' }}>
                  Student: <strong>{selectedEnrollment.student_name}</strong>
                </p>
              </div>
              <button
            ...
            {loadingDetail ? (
              ...
              <>
                {/* Schedule Section */}
                {(() => {
                  const classInfo = classes.find(c => c.id === selectedEnrollment.tutoring_class);
                  ...
                  return (
                    <div style={{
                      ...
                      <h3 style={{ margin: '0 0 1rem 0', color: '#192A88', fontSize: '1.2rem' }}>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem' }}>
                        ...
                        <div>
                          ...
                          <p style={{ margin: '0.25rem 0 0 0' }}>
                            {classInfo.difficulty?.replace('_', ' ').toUpperCase() || 'Not set'}
                          </p>
                        </div>
                      </div>
                ...
                <div style={{
                  ...
                  {enrollmentFiles.length === 0 ? (
                    <p style={{ color: '#666', margin: 0 }}>No files available for this class yet.</p>
                  ) : (
                ...
                <div style={{
                  ...
                  {enrollmentQuizzes.length === 0 ? (
                    <p style={{ color: '#666', margin: 0 }}>No quizzes available for this class yet.</p>
                  ) : (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                      {enrollmentQuizzes.map(quiz => (
                        <div
                          key={quiz.id}
                          style={{
                            padding: '1rem',
                        ...
                        >
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                            <div>
                              <div style={{ fontWeight: '500', color: '#333', fontSize: '1.05rem' }}>
                                {quiz.title}
                              </div>
                              {quiz.description && (
                              ...
                              <div style={{ fontSize: '0.85rem', color: '#666', marginTop: '0.5rem' }}>
                                ...
                                {quiz.time_limit_minutes && (
                                  <span style={{ marginLeft: '1rem' }}>Time Limit: {quiz.time_limit_minutes} min</span>
                                )}
                                <span style={{ marginLeft: '1rem' }}>Passing Score: {quiz.passing_score}%</span>
                              </div>
                            </div>
